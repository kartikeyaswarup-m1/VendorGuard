...
# Lazy initialization - check at runtime
...
def classify_control_with_gemini(control_id, control_text, evidences, max_output_tokens=2048, temperature=0.0):
    ...
    if client is None:
        return {
            ...
            "confidence": 0.0,
            "rationale": f"LLM provider not configured (missing API key). LLM_PROVIDER={LLM_PROVIDER}, GOOGLE_API_KEY={'set' if GOOGLE_API_KEY else 'not set'}",
            "evidence": evidences,
            "followup_questions": []
        }
