...
export default function Home(){
  ...
  useEffect(() => {
    fetch("http://localhost:8000/api/controls/frameworks")
      .then(res => res.json())
  ...
  useEffect(() => {
    if (uploaded?.vendor_id) {
      fetch(`http://localhost:8000/api/history/${uploaded.vendor_id}?limit=5`)
        .then(res => res.json())
  ...
  async function runAnalyze(vendorId, filePaths) {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/analyze/${vendorId}`, {
        method: "POST",
